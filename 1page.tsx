...
// ============================================================
// Brewing Brothers — Production Funnel Page v2.0
// CF Patterns: P1 Hero · P3 Offer Stack · P4 Social Proof
//              P5 Urgency Timer · P8 Sticky CTA
// Edit all brand data in BRAND_CONFIG below
// ============================================================
...
export const PRODUCTS = [
  {
    name: "Single Bottle",
    price: process.env.NEXT_PUBLIC_PRICE_SINGLE || "$12",
    per:  "$12/bottle",
    note: "Perfect intro — taste the freshness.",
    badge: null,
  ...
  {
    name: "6-Pack",
    price: process.env.NEXT_PUBLIC_PRICE_SIXPACK || "$64",
    per:  "$10.67/bottle",
  ...
  {
    name: "12-Pack",
    price: process.env.NEXT_PUBLIC_PRICE_TWELVE || "$119",
    per:  "$9.92/bottle",
    note: "Best value — stock up, free shipping.",
    badge: "BEST VALUE",
...
export const TESTIMONIALS = [
  {
    quote: "Best juice I've had. You can taste the freshness the second you open it. Won't buy store-bought again.",
    name: "Sarah M.", location: "Sacramento, CA", product: "6-Pack", stars: 5,
  },
  {
    quote: "Finally a local juice worth driving for. The cold-press process is real — you can tell the difference.",
    name: "James T.", location: "Orangevale, CA", product: "12-Pack", stars: 5,
  },
  {
    quote: "Ordered twice already. My whole family loves it. The 12-pack disappears in a week.",
    name: "Lisa R.", location: "Folsom, CA", product: "12-Pack", stars: 5,
  },
];
...
export default function Home() {
  ...
  // Sticky mobile bar — activates at 400px scroll depth
  useEffect(() => {
  ...
  async function handleOrder(priceId: string) {
    ...
    if (!enabled) {
      alert("Online ordering is coming soon! Reserve a local pickup below — no payment required.");
      return;
    }
    try {
  ...
  return (
    <main className={`min-h-screen ${t.pageBg}`}>
      ...
      {/* CF-P8 Sticky Mobile Bar — scroll-activated */}
      <div
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        {/* CF-P5 Urgency Bar — countdown timer */}
        <div className="rounded-2xl bg-amber-900 text-amber-50 px-5 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 mb-8">
          <p className="text-sm font-medium">
            🧃 <strong>{BRAND_CONFIG.urgencyMsg}</strong> Reserve before it&apos;s gone.
          </p>
        ...
        {/* CF-P1 Hero — squeeze page, no nav, dual CTA above fold */}
        <div className={`rounded-3xl p-8 sm:p-12 ${t.card}`}>
          <div className="flex flex-col gap-5 max-w-3xl">
            ...
            <h1 className={`text-4xl sm:text-5xl font-bold leading-tight ${t.accent}`}>
              ...
              <br />
              <span className="text-amber-600">{BRAND_CONFIG.tagline}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <a
                href="#order"
                className={`inline-flex justify-center items-center px-8 py-4 rounded-2xl font-bold text-lg shadow-lg ${t.button}`}
              >
                {BRAND_CONFIG.primaryCTA}
              </a>
              <a
                href="#pickup"
                className="inline-flex justify-center items-center px-8 py-4 rounded-2xl font-bold text-lg border-2 border-amber-700 text-amber-800 bg-white hover:bg-amber-50 transition"
              >
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {TRUST_BADGES.map((b) => (
            <div key={b.label} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${t.badge}`}>
          ...
          <div className={`rounded-3xl p-8 ${t.card}`}>
            ...
            <p className="mt-1 text-slate-600">
              {process.env.NEXT_PUBLIC_STRIPE_ENABLED === "true"
                ? "Secure Stripe payment. Ships within 24 hours."
                : "Stripe checkout coming soon — reserve a pickup below!"}
            </p>
              ...
              {PRODUCTS.map((p) => (
                <div
                  ...
                  className={`relative rounded-2xl border-2 p-6 flex flex-col bg-white ${
                    p.badge === "MOST POPULAR" ? "border-amber-600 shadow-lg" : "border-slate-200"
                  }`}
                >
                  {p.badge && (
                    <span
                      className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-xs font-bold ${
                        p.badge === "MOST POPULAR" ? "bg-amber-600 text-white" : "bg-slate-800 text-white"
                      }`}
                    >
                      {p.badge}
                    </span>
                  ...
                  <button
                    onClick={() => handleOrder(p.priceId)}
                    className={`w-full px-4 py-3 rounded-xl font-bold ${t.button}`}
                  >
            ...
            <p className="mt-5 text-sm text-slate-500 text-center">
              ✅ Secure Stripe checkout · 📦 USA shipping only · 🔄 Satisfaction guarantee
            </p>
          </div>
        </section>
        ...
        <div className="mt-6 text-center">
          <a href="#pickup" className="inline-flex justify-center items-center px-8 py-3 rounded-2xl font-semibold border-2 border-amber-700 text-amber-800 bg-white hover:bg-amber-50 transition">
            Prefer local pickup? Reserve below →
          </a>
        </div>
          ...
          <div className={`rounded-3xl p-8 ${t.card}`}>
            ...
            <form onSubmit={submitPickup} className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              >
                ...
                <option>Custom — specify at pickup</option>
              </select>
              <div className="sm:col-span-2">
                <button
                  disabled={loading}
                  className={`w-full px-6 py-4 rounded-2xl font-bold text-lg ${t.button} disabled:opacity-60`}
                >
                  {loading ? "Submitting…" : "Reserve My Pickup Spot →"}
                </button>
              </div>
              {msg && (
                <div className="sm:col-span-2 text-sm text-red-700 bg-red-50 rounded-xl p-3">{msg}</div>
              )}
            </form>
            <p className="mt-4 text-xs text-slate-500 text-center">
              We&apos;ll reach out within 24 hours to confirm. No payment required until pickup.
            </p>
          </div>
        </section>
        ...
        {/* Final CTA — dark amber, maximum contrast */}
        <section className="mt-10">
          <div className="rounded-3xl bg-amber-900 p-10 text-center">
            ...
            <p className="text-amber-200 mb-8 max-w-lg mx-auto">
              Order for shipping or reserve your local pickup spot today. Small-batch means limited availability every week.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#order"  className="inline-flex justify-center items-center px-10 py-4 rounded-2xl font-bold text-lg bg-white text-amber-900 hover:bg-amber-50 transition shadow-lg">
                Order Now — Ships USA
              </a>
              <a href="#pickup" className="inline-flex justify-center items-center px-10 py-4 rounded-2xl font-bold text-lg border-2 border-white text-white hover:bg-amber-800 transition">
                Reserve Pickup (Free)
              </a>
            </div>
            <p className="text-amber-300 text-xs mt-6">
              🔒 Secure · 🌿 Organic · 📍 {BRAND_CONFIG.location} · 🚚 {BRAND_CONFIG.shipping}
            </p>
          </div>
        </section>

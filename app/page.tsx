...
export const PRODUCTS = [
  {
    name: "1-Pack (20oz)",
    price: process.env.NEXT_PUBLIC_PRICE_SINGLE || "$16",
    per: "$16 per bottle",
  ...
  {
    name: "2-Pack (20oz)",
    price: process.env.NEXT_PUBLIC_PRICE_SIXPACK || "$28",
    per: "$14 per bottle - save 12%",
  ...
  {
    name: "3-Pack (20oz)",
    price: process.env.NEXT_PUBLIC_PRICE_TWELVE || "$36",
    per: "$12 per bottle - best value",
    note: "Best value - stock up and save.",
    badge: "BEST VALUE",
...
export const TESTIMONIALS = [
  { quote: "Best juice I've had. You can taste the freshness the second you open it. Won't buy store-bought again.", name: "Sarah M.", location: "Sacramento, CA", product: "2-Pack", stars: 5 },
  { quote: "Finally a local juice worth driving for. The cold-press process is real - you can tell the difference.", name: "James T.", location: "Orangevale, CA", product: "3-Pack", stars: 5 },
  { quote: "Ordered twice already. My whole family loves it.", name: "Lisa R.", location: "Folsom, CA", product: "3-Pack", stars: 5 },
];
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className={`rounded-3xl p-8 sm:p-12 ${t.card}`}>
          <div className="flex flex-col gap-5 max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <a href="#order" className={`inline-flex justify-center items-center px-8 py-4 rounded-2xl font-bold text-lg shadow-lg ${t.button}`}>
                {BRAND_CONFIG.primaryCTA}
              </a>
              <a href="#pickup" className="inline-flex justify-center items-center px-8 py-4 rounded-2xl font-bold text-lg border-2 border-amber-700 text-amber-800 bg-white hover:bg-amber-50 transition">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-3">
              {PRODUCTS.map((p) => (
                <div key={p.name} className={`relative rounded-2xl border-2 p-6 flex flex-col bg-white ${p.badge === "MOST POPULAR" ? "border-amber-600 shadow-lg" : "border-slate-200"}`}>
                  {p.badge && (
                    <span className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-xs font-bold ${p.badge === "MOST POPULAR" ? "bg-amber-600 text-white" : "bg-slate-800 text-white"}`}>
                      {p.badge}
                    </span>
                  )}
        ...
        <section id="pickup" className="mt-14">
          <div className={`rounded-3xl p-8 ${t.card}`}>
            ...
            <p className="mt-1 text-slate-600">Reserve now. Pay on pickup. We will contact you to confirm timing.</p>
            <form onSubmit={submitPickup} className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <select name="quantity" defaultValue="" className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" required>
                ...
                <option>1-Pack (20oz)</option>
                <option>2-Pack (20oz)</option>
                <option>3-Pack (20oz)</option>
            ...
            <p className="mt-4 text-xs text-slate-500 text-center">
              We will reach out within 24 hours to confirm. No payment required until pickup.
            </p>
          </div>
        </section>
